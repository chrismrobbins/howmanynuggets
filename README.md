...
| `js/botsSim.js` | 🤖 BatteredBots RULEBOOK — deterministic shared sim (SP authority, worker authority, online predictor); change rules here |
| `js/bots.js` | 🤖 BatteredBots shell + WebGL renderer (normal-mapped floor, 16 point lights, decal layer, bloom; procedural stand-ins for every sprite) |
| `js/botsMP.js` | 🤖 CLUCKED METAL online adapter (snapshots, own-bot prediction, remote interpolation) |
| `js/botsArt.js` | GENERATED by `blender/pack_bots.py` — Blender sprite pages (albedo/normal/PAINT mask) + arena floor pages, injected async |
| `css/bots.css` | BatteredBots layer + online pill |
