...
def main():
    ...
    # Two files: the LOADER (js/hallArt.js, hand-maintained, tiny, in
    # index.html) and the DATA (js/hallArtData.js, generated, ~450KB, injected
    # async after first paint). Only the data half is written from here — the
    # loader is real code with a fallback contract and does not want a
    # generator stomping it. See blender/HANDOFF.md §10.
    data = DATA_TEMPLATE.replace(
        "__REGIONS__", json.dumps(regions, sort_keys=True, separators=(",", ":")))
    data = data.replace("__B64__", b64)
    out_js = os.path.join(repo, "js", "hallArtData.js")
    ...
    print("wrote", out_js)
    print("NOTE: js/hallArt.js is the hand-written loader — not regenerated here.")


DATA_TEMPLATE = r"""/* js/hallArtData.js — the hall's Blender-rendered paint, as one JPEG.
 *
 * GENERATED by blender/pack_hall.py. Do not hand-edit.
 *
 * NOT in index.html: js/hallArt.js injects this as an async <script> after
 * first paint, so ~450KB of arcade art never delays the converter. The arcade
 * boot screen (js/hallBoot.js) is where the waiting happens.
 */
window.__HALL_ART__ = {r:__REGIONS__,s:'__B64__'};
"""
